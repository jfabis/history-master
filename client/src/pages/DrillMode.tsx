...
import { ArrowLeft, Scroll, CheckCircle, XCircle, Trophy, HelpCircle, BookOpen } from 'lucide-react';
    ...
    <div className="min-h-screen bg-[#f0e6d2] font-serif text-[#2c241b] relative overflow-hidden">
      ...
      <nav className="relative z-10 px-6 py-4 flex items-center justify-between border-b border-[#c5a059]/30">
        ...
        <div className="flex items-center gap-2 text-[#8b1e1e]">
          ...
          <span className="font-bold text-lg">Drill & Practice</span>
        </div>
      </nav>
      ...
      <main className="relative z-10 max-w-4xl mx-auto p-6 flex flex-col items-center justify-center min-h-[80vh]">
        ...
        {stage === 'QUIZ' && questions.length > 0 && (
          <div className="w-full max-w-2xl">
            <div className="mb-8 flex justify-between items-end border-b border-[#c5a059] pb-4">
              ...
              <span className="text-[#8b1e1e] font-bold">{totalXP} XP</span>
            </div>
              ...
              <div className="space-y-4">
                {questions[currentIndex].options.map((option) => (
                  <button
                    ...
                    disabled={!!feedback}
                    className={`w-full p-4 text-left border-2 rounded-sm transition-all duration-200 font-serif text-lg
                      ${feedback 
                        ? option === feedback.correctAnswer 
                          ? 'bg-green-50 border-green-600 text-green-800'
                          : option === questions[currentIndex].options.find(o => o === option) && !feedback.correct 
                            ? 'opacity-50' 
                            : 'opacity-50'
                        : 'border-[#d4c5a6] hover:border-[#8b1e1e] hover:bg-[#fff9f0] text-[#2c241b]'
                      }
                      ${feedback && !feedback.correct && feedback.correctAnswer !== option ? '' : ''}
                    `}
                  >
                    {option}
                  </button>
        ...
        {stage === 'SUMMARY' && (
          <div className="text-center max-w-lg w-full bg-[#fdfbf7] p-12 border-4 border-double border-[#d4c5a6] shadow-2xl relative">
             ...
             <div className="text-[#5c4d3c] font-serif italic mb-10">Punktów Chwały (XP)</div>
